...
class ResidualAttentionBlock_MaPLePlus(nn.Module):
    def __init__(self, 
                 d_model: int, 
                 n_head: int, 
    ...
    def forward(self, inputs):
        ...
        x = inputs[0]
        compound_prompts_deeper = inputs[1]
        compound_prompts_deeper_condition = inputs[2]
        counter = inputs[3]
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if counter > len(compound_prompts_deeper) - 1:
                        ...
                        visual_context_condition = compound_prompts_deeper_condition[counter]
                        visual_context_condition = visual_context_condition.expand(
                            x.shape[1], -1, -1).permute(1, 0, 2).half()
                        # Add the learnable tokens of this layer with the input, by replacing previous
                        # layer learnable tokens
                        x = torch.cat([prefix, visual_context, visual_context_condition], dim=0)
                    ...
                    if counter > len(compound_prompts_deeper) - 1:
                        ...
                        textual_context_condition = compound_prompts_deeper_condition[counter]
                        textual_context_condition = textual_context_condition.expand(
                            x.shape[1], -1, -1).permute(1, 0, 2).half()
                        # Add the learnable tokens of this layer with the input, replaced by previous
                        # layer learnable tokens
                        x = torch.cat([prefix, textual_context, textual_context_condition, suffix], dim=0)
                        # Once done, update the counter, so that the next time, it does not use same learnable tokens
                        counter += 1
        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))
        return [x, compound_prompts_deeper, compound_prompts_deeper_condition, counter]  # return again as a list, so that nn.seq can work


class ResidualAttentionBlock_MaPLePlusCon(nn.Module):
    def __init__(self, 
                 d_model: int, 
                 n_head: int, 
                 attn_mask: torch.Tensor = None, 
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer == 'MaPLePlus':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLePlus(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
...
class VisionTransformer_MaPLePlus(nn.Module):
    def __init__(self, 
    ...
    def forward(self, 
                ...
                compound_deeper_prompts_condition):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        outputs = self.transformer([
            x, compound_deeper_prompts, compound_deeper_prompts_condition, 0])  # the last argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            elif trainer == "MaPLePlus":
                self.visual = VisionTransformer_MaPLePlus(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
